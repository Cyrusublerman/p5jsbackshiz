...
$$G_{A}={\frac {N_{C}-N_{V}}{N_{V}}}
$$, where $N_{V}>0$ is the total number of asymmetric vectors, $N_{C}$ is the number of Delaunay connections among them and the property $N_{C}>N_{V}$ is valid for any gradient square lattice.
