...
In computer science, the B tree is a query that is used to update efficient B+ tree-based index structures for moving objects.
...
Finally, with the combination of the partition number (time information) and the linear order (location information), an object is indexed in B-tree with a one-dimensional index key B value:


$$B^{x}{\text{ value}}\left(O,t\right)=\left[{\text{index partition}}\right]_{2}+\left[{\text{xrep}}\right]_{2}
$$

Here index-partition is an index partition determined by the update time and xrep is the space-filling curve value of the object position at the indexed time, $\left[X\right]_{2}$ denotes the binary value of x, and “+” means concatenation.
...
4. Concatenating indexpartition and xrep, Bvalue (00010011)2=19.
...
$$at time $tq$ not prior to the current time.
