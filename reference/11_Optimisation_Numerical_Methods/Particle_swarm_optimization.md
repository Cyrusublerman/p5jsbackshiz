...
Let S be the number of particles in the swarm, each having a position xi ∈ ℝ in the search-space and a velocity vi ∈ ℝ. Let pi be the best known position of particle i and let g be the best known position of the entire swarm. A basic PSO algorithm to minimize the cost function is then:
...
$${\vec {x}}_{i}=G\left({\frac {{\vec {p}}_{i}+{\vec {g}}}{2}},||{\vec {p}}_{i}-{\vec {g}}||\right)\,,
$$

where ${\vec {x}}_{i}$, ${\vec {p}}_{i}$ are the position and the best position of the particle $i$; ${\vec {g}}$ is the global best position; $G({\vec {x}},\sigma )$ is the normal distribution with the mean ${\vec {x}}$ and standard deviation $\sigma$; and where $||\dots ||$ signifies the norm of a vector.
...
- Simulation of PSO convergence in a two-dimensional space (Matlab). Archived 2024-04-14 at the Wayback Machine
- Applications of PSO.
...
- Links to PSO source code Archived 2021-04-15 at the Wayback Machine
