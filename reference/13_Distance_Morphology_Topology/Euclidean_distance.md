...
In the Euclidean plane, let point $p$ have Cartesian coordinates $(p_{1},p_{2})$ and let point $q$ have coordinates $(q_{1},q_{2})$. Then the distance between $p$ and $q$ is given by:
...
$$d(p,q)=(p_{1}-q_{1})\oplus (p_{2}-q_{2})={\mathsf {hypot}}(p_{1}-q_{1},p_{2}-q_{2}).
$$
...
- It obeys the triangle inequality: for every three points $p$, $q$, and $r$, $d(p,q)+d(q,r)\geq d(p,r)$. Intuitively, traveling from $p$ to $r$ via $q$ cannot be any shorter than traveling directly from $p$ to $r$.
...
- Minkowski distance (L distance), a generalization that unifies Euclidean distance, taxicab distance, and Chebyshev distance.
