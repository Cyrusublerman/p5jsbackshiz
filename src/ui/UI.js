...
export class UI {
  ...
  _progressInterval() {
    ...
    setInterval(() => {
      if (this.state.rendering) {
        bar.style.width = (this.state.renderProgress * 100) + '%';
      } else {
        bar.style.width = '0%';
      }
    }, 50);
  ...
  _menu() {
    ...
    for (const [group, items] of Object.entries(REGISTRY)) {
      ...
      for (const item of items) {
        ...
        mi.className = 'mi';
        mi.dataset.label = item.label.toLowerCase();
        mi.dataset.group = group.toLowerCase();
    ...
    search.addEventListener('input', () => {
      ...
      for (const item of allItems) {
        ...
        if (match) groups.add(item.groupEl);
      }
      menu.querySelectorAll('.mg').forEach(g => {
        g.style.display = groups.has(g) || !q ? '' : 'none';
      });
    });
  ...
  _fnKeys() {
    ...
    fk('fk2', () => {
      ...
      this.state.needsRender = true; this.onRender(); this.statusPanel.update(this.state);
    });
    fk('fk3', () => {
  ...
  _presetMenu() {
    ...
    document.addEventListener('click', (e) => {
      if (!menu.contains(e.target) && e.target?.id !== 'fk-presets') menu.classList.remove('show');
    });
  }
  ...
  _renderVariations(grid) {
    ...
    const origQuality = this.state.quality;
    const origScale = this.state.previewScale;
    this.state.quality = 'preview';
    this.state.previewScale = 0.2;

    for (let i = 0; i < 9; i++) {
      const seed = baseSeed + i * 7919;
      this.state.globalSeed = seed;
      this.state.needsRender = true;
      // Invalidate all caches for new seed
      ...
      seedLbl.className = 'vg-seed';
      seedLbl.textContent = seed;
      cell.appendChild(seedLbl);
      ...
      cell.addEventListener('click', () => {
        ...
        this.state.quality = origQuality;
        this.state.previewScale = origScale;
        this.onRender();
      });
    ...
    this.state.globalSeed = baseSeed;
    this.state.quality = origQuality;
    this.state.previewScale = origScale;
  }
  ...
  _keyboard() {
    document.addEventListener('keydown', (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && key === 'z') {
        e.preventDefault();
        if (e.shiftKey) {
          if (this.history.redo(this.state, REGISTRY)) { this.refreshStack(); this.state.needsRender = true; this.onRender(); }
        } else {
          if (this.history.undo(this.state, REGISTRY)) { this.refreshStack(); this.state.needsRender = true; this.onRender(); }
        }
        return;
      }
      ...
      // Node operations
      const s = this.state;
      const idx = s.selectedNodeIdx;

      if (key === 'a') { document.getElementById('add-btn').click(); return; }
      if (key === 'v') { const el = document.getElementById('fk-vargrid'); if (el) el.click(); return; }

      if (key === 'tab') {
        e.preventDefault();
        if (s.stack.length === 0) return;
        if (e.shiftKey) s.selectedNodeIdx = Math.max(0, idx - 1);
        else s.selectedNodeIdx = Math.min(s.stack.length - 1, idx + 1);
        this.refreshStack();
        return;
      }
      ...
      if (idx >= 0 && idx < s.stack.length) {
        ...
        if (key === 'e') {
          ...
          s.needsRender = true;
          this.refreshStack(); this.onRender();
        } else if (key === 'd') {
          this._snap();
          s.stack.splice(idx, 1);
          if (s.soloNodeId === node.id) s.soloNodeId = null;
          s.selectedNodeIdx = Math.min(idx, s.stack.length - 1);
          s.needsRender = true;
          this.refreshStack(); this.onRender();
        } else if (key === '[' && idx > 0) {
          this._snap();
          [s.stack[idx], s.stack[idx - 1]] = [s.stack[idx - 1], s.stack[idx]];
          s.selectedNodeIdx = idx - 1;
          s.needsRender = true;
          this.refreshStack(); this.onRender();
        } else if (key === ']' && idx < s.stack.length - 1) {
          this._snap();
          [s.stack[idx], s.stack[idx + 1]] = [s.stack[idx + 1], s.stack[idx]];
          s.selectedNodeIdx = idx + 1;
          s.needsRender = true;
  ...
  _modMapUpload() {
    ...
    document.getElementById('mask-input').addEventListener('change', (e) => {
      const file = e.target.files[0]; const node = this._maskTargetNode;
      if (!file || !node) return;
      const reader = new FileReader();
      reader.onload = (ev) => {
        const img = new Image();
        img.onload = () => {
          ...
          node.mask._sourceW = img.width; node.mask._sourceH = img.height;
          node.mask.source = 'upload'; node.mask.enabled = true;
  ...
  _transport() {
    ...
    const updateTransport = () => {
      ...
      if (s.frameCount > 1) {
        ...
        scrub.max = s.frameCount - 1;
        scrub.value = s.currentFrame;
        infoFrame.textContent = `${s.currentFrame + 1}/${s.frameCount}`;
      } else {
    ...
    scrub.addEventListener('input', () => {
      this.state.seekFrame(parseInt(scrub.value));
      this.onRender();
      updateTransport();
    });
    ...
    playBtn.addEventListener('click', () => {
      ...
      if (s.isPlaying) {
        ...
        animInterval = setInterval(() => {
          s.seekFrame((s.currentFrame + 1) % s.frameCount);
          this.onRender();
          updateTransport();
        }, 1000 / s.fps);
      }
    ...
    document.getElementById('t-step-fwd')?.addEventListener('click', () => {
      this.state.seekFrame(this.state.currentFrame + 1);
      this.onRender(); updateTransport();
    });
    document.getElementById('t-step-back')?.addEventListener('click', () => {
      this.state.seekFrame(this.state.currentFrame - 1);
      this.onRender(); updateTransport();
    });
    document.getElementById('t-prev')?.addEventListener('click', () => {
      this.state.seekFrame(0); this.onRender(); updateTransport();
    });
    document.getElementById('t-next')?.addEventListener('click', () => {
      this.state.seekFrame(this.state.frameCount - 1); this.onRender(); updateTransport();
    });
  }
  ...
  _dragDrop() {
    ...
    window.addEventListener('mousemove', (e) => {
      if (pan) { this.state.panX += e.clientX - lx; this.state.panY += e.clientY - ly; lx = e.clientX; ly = e.clientY; }
    ...
    vp.addEventListener('wheel', (e) => {
      e.preventDefault();
      if (this.state.zoom === 'fit') { this.state.zoom = 'custom'; this.state.zoomLevel = 1; }
  ...
  _exportPNG() {
    ...
    const a = document.createElement('a'); a.download = `distort_${this.state.globalSeed}.png`;
  ...
  _exportJSON() {
    ...
    const a = document.createElement('a'); a.download = `recipe_${this.state.globalSeed}.json`;
  ...
  /* ── Stack rebuild (with selection highlight + delete safety) ── */
  refreshStack() {
    const el = document.getElementById('stack');
    const scrollTop = el.scrollTop; // preserve scroll
    el.innerHTML = '';
    ...
    this.state.stack.forEach((node, idx) => {
      ...
      card.addEventListener('click', () => { this.state.selectedNodeIdx = idx; this.refreshStack(); });
      ...
      const maskBtn = document.createElement('button'); maskBtn.className = 'nb mask-btn' + (node.mask.enabled ? ' on' : ''); maskBtn.textContent = 'M'; maskBtn.title = 'Mask';
      ...
      // Dice (randomise all params)
      const diceBtn = document.createElement('button'); diceBtn.className = 'nb dice'; diceBtn.textContent = '\u2684'; diceBtn.title = 'Randomise params';
      diceBtn.addEventListener('click', (e) => {
        e.stopPropagation(); this._snap();
        for (const [k, def] of Object.entries(node.paramDefs)) {
          if (def.type === 'select') { node.params[k] = def.options[Math.floor(Math.random() * def.options.length)]; }
          else { node.params[k] = def.min + Math.random() * (def.max - def.min); }
        }
      ...
      // Delete button at bottom of expanded params (safe position)
      if (node.expanded) {
        ...
        delBtn.addEventListener('click', (e) => {
          ...
          this.state.selectedNodeIdx = Math.min(idx, this.state.stack.length - 1);
          this.state.needsRender = true; this.refreshStack(); this.onRender();
        });
  ...
  _maskControls(container, node) {
    ...
    for (const opt of ['luminance', 'gradient', 'upload']) {
      ...
      if (node.mask.source === opt) o.selected = true; srcSel.appendChild(o);
    }
    srcSel.addEventListener('change', () => {
      if (srcSel.value === 'upload') { this._maskTargetNode = node; document.getElementById('mask-input').click(); }
      else { node.mask.source = srcSel.value; node.invalidate(this.state.stack); this.state.needsRender = true; this.onRender(); }
    });
    ...
    this._p(container, node, '_feather', { value: node.mask.feather, min: 0, max: 20, step: 0.5, label: 'FEATHER' },
      (v) => { node.mask.feather = v; node.invalidate(this.state.stack); });
  }
  ...
  _modRow(container, node, key, def) {
    ...
    sel.addEventListener('change', () => {
      if (sel.value) node.modulation[key] = { mapId: sel.value, amount: node.modulation[key]?.amount || 1 };
      else delete node.modulation[key];
      node.invalidate(this.state.stack); this.state.needsRender = true; this.refreshStack(); this.onRender();
    });
    const amtSl = document.createElement('input'); amtSl.type = 'range'; amtSl.className = 'ps'; amtSl.min = 0; amtSl.max = 1; amtSl.step = 0.01; amtSl.value = mod.amount || 0;
    amtSl.addEventListener('input', () => {
      ...
      node.invalidate(this.state.stack); this.state.needsRender = true; this.onRender();
    });
    ...
    if (prevRow && prevRow.classList.contains('pr')) {
      const dot = document.createElement('span'); dot.className = 'mod-dot' + (mod.mapId ? ' on' : ''); dot.title = 'Modulation';
      dot.addEventListener('click', (e) => { e.stopPropagation(); row.classList.toggle('hide'); });
  ...
  _p(container, node, key, def, setter) {
    ...
    if (def.type === 'select') {
      ...
      sl.addEventListener('input', () => {
        const v = parseFloat(sl.value); setter(v);
      ...
      sl.addEventListener('change', () => {
        this._snap();
        if (this._savedScale) { this.state.previewScale = this._savedScale; this._savedScale = null; this.state.needsRender = true; node.invalidate(this.state.stack); this.onRender(); }
      });
      num.addEventListener('change', () => {
        this._snap();
        let v = Math.max(def.min, Math.min(def.max, parseFloat(num.value))); setter(v); sl.value = v;
        num.value = v.toFixed(def.step < 1 ? 2 : 0);
        node.invalidate(this.state.stack); this.state.needsRender = true; this.onRender();
      });

      // Per-param dice
      const dice = document.createElement('button'); dice.className = 'nb dice'; dice.textContent = '\u2684'; dice.title = 'Randomise';
      dice.addEventListener('click', (e) => {
        e.stopPropagation(); this._snap();
        const rv = def.min + Math.random() * (def.max - def.min);
        setter(rv); sl.value = rv; num.value = rv.toFixed(def.step < 1 ? 2 : 0);
        node.invalidate(this.state.stack); this.state.needsRender = true; this.onRender();
      });
